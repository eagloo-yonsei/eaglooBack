...
import { sendMail, sendResetMail } from "src/utils/sendMail";
import { secretGenerator } from "src/utils/secretGenerator";
...
export class UserService {
  ...
  async resetPassword(email: string, secret: string, newPassword: string) {
    try {
      const user = await prisma.user.findUnique({ where: { email } });
    ...
    return {
      success: true,
      message: "유저 비밀번호가 성공적으로 변경되었습니다",
    };
  }
}
